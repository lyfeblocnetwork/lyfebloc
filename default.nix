...
let
  ...
  ldflags = lib.concatStringsSep "\n" ([
    "-X github.com/cosmos/cosmos-sdk/version.Name=lyfebloc"
...
buildGoApplication rec {
  ...
  subPackages = [ "cmd/lyfeblocd" ];
  CGO_ENABLED = "1";

  postFixup = if dbBackend == "rocksdb" then
    ''
      # Rename the binary from lyfeblocd to lyfeblocd-rocksdb
      mv $out/bin/lyfeblocd $out/bin/lyfeblocd-rocksdb
    '' else '''';

  meta = with lib; {
    description = "Lyfebloc is a scalable and interoperable blockchain, built on Proof-of-Stake with fast-finality using the Cosmos SDK which runs on top of CometBFT Core consensus engine.";
    homepage = "https://github.com/lyfeblocnetwork/lyfebloc";
    license = licenses.asl20;
    mainProgram = "lyfeblocd";
  };
}
