...
import (
	...
	lyfeblocutiltx "github.com/lyfeblocnetwork/lyfebloc/testutil/tx"
...
var (
	...
	toAddr           = lyfeblocutiltx.GenerateAddress()
	funderAddr       = lyfeblocutiltx.GenerateAddress()
	diffFunderAddr   = lyfeblocutiltx.GenerateAddress()
	...
	testCases := []struct {
		...
		},
		{
			...
			func() []interface{} {
				...
				err = lyfeblocutil.FundAccount(ctx, s.network.App.BankKeeper, toAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, math.NewInt(100))))
				return []interface{}{
					s.keyring.GetAddr(0),
					toAddr,
					uint64(time.Now().Unix()), //nolint:gosec // G115
	...
	testCases := []struct {
		...
		},
		{
			...
			func(data []byte) {
				...
				_, ok := account.(lyfebloctypes.EthAccountI)
				s.Require().True(ok)
			},
			false,
			"",
