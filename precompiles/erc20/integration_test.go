...
import (
	...
	auth "github.com/lyfeblocnetwork/lyfebloc/precompiles/authorization"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/erc20"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/erc20/testdata"
	...
	"github.com/lyfeblocnetwork/lyfebloc/testutil/integration/lyfebloc/utils"
...
var (
	wlyfeblocAddress      common.Address
...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		Expect(common.HexToAddress(erc20Params.NativePrecompiles[0])).To(Equal(common.HexToAddress(erc20types.WLYFEBLOCContractTestnet)))

		wlyfeblocAddress = common.HexToAddress(erc20Params.NativePrecompiles[0])
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the WLYFEBLOC contract", func() {
				var (
				...
				DescribeTable("it should revert token transfer from the WLYFEBLOC contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
				...
				It("it should send token transfer and send from WLYFEBLOC contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from WLYFEBLOC contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					balRes, err := is.handler.GetBalance(receiver.Bytes(), is.bondDenom)
					Expect(err).To(BeNil())
...
var _ = Describe("ERC20 Extension migration Flows -", func() {
	...
	When("using Lyfebloc (not wLyfebloc) in smart contracts", func() {
