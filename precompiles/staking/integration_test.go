...
import (
	...
	compiledcontracts "github.com/lyfeblocnetwork/lyfebloc/contracts"
	"github.com/lyfeblocnetwork/lyfebloc/crypto/ethsecp256k1"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/authorization"
	cmn "github.com/lyfeblocnetwork/lyfebloc/precompiles/common"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/distribution"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/staking"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/staking/testdata"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/testutil"
	"github.com/lyfeblocnetwork/lyfebloc/precompiles/testutil/contracts"
	lyfeblocutil "github.com/lyfeblocnetwork/lyfebloc/testutil"
	...
	testutils "github.com/lyfeblocnetwork/lyfebloc/testutil/integration/lyfebloc/utils"
...
var _ = Describe("Calling staking precompile directly", func() {
	...
	Describe("Calling precompile txs from a vesting account", func() {
		...
		BeforeEach(func() {
			...
			vestingAmtTotal := lyfeblocutil.TestVestingSchedule.TotalVestingCoins
		...
		Context("before first vesting period - all tokens locked and unvested", func() {
			BeforeEach(func() {
				...
				Expect(unvested).To(Equal(lyfeblocutil.TestVestingSchedule.TotalVestingCoins), "expected different unvested coins")
			...
			It("Should be able to delegate tokens not involved in vesting schedule", func() {
				...
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(lyfeblocutil.TestVestingSchedule.TotalVestingCoins[0]).Add(coinsToDelegate[0])))
				...
				Expect(err).To(BeNil())
				Expect(balRes.Balance.IsGTE(lyfeblocutil.TestVestingSchedule.TotalVestingCoins[0])).To(BeTrue())
			})
		})
		...
		Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
			BeforeEach(func() {
				...
				cliffDuration := time.Duration(lyfeblocutil.TestVestingSchedule.CliffPeriodLength)
				Expect(s.network.NextBlockAfter(cliffDuration * time.Second)).To(BeNil())
				ctx := s.network.GetContext()

				// Check if some, but not all tokens are vested
				vested = clawbackAccount.GetVestedCoins(ctx.BlockTime())
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, lyfeblocutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(lyfeblocutil.TestVestingSchedule.CliffMonths))))
				Expect(vested).NotTo(Equal(lyfeblocutil.TestVestingSchedule.TotalVestingCoins), "expected some tokens to have been vested")
			...
			It("Should be able to delegate locked vested tokens + free tokens (not in vesting schedule)", func() {
				...
				balRes, err := s.grpcHandler.GetBalance(vestAcc.Bytes(), s.bondDenom)
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(lyfeblocutil.TestVestingSchedule.TotalVestingCoins[0]).Add(amt[0])))
		...
		Context("Between first and second lockup periods - vested coins are unlocked", func() {
			BeforeEach(func() {
				...
				vestDuration := time.Duration(lyfeblocutil.TestVestingSchedule.LockupPeriodLength)
				...
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, lyfeblocutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(lyfeblocutil.TestVestingSchedule.LockupMonths))))
				expUnlockedVested := expVested

				Expect(vested).NotTo(Equal(lyfeblocutil.TestVestingSchedule.TotalVestingCoins), "expected not all tokens to be vested")
				...
				Expect(unlocked).To(Equal(lyfeblocutil.TestVestingSchedule.UnlockedCoinsPerLockup))
	...
	Context("delegating", func() {
		...
		Describe("delegation from a vesting account", func() {
			...
			BeforeEach(func() {
				...
				// Check if all tokens are unvested at vestingStart
				totalVestingCoins := lyfeblocutil.TestVestingSchedule.TotalVestingCoins
				unvested = clawbackAccount.GetVestingCoins(ctx.BlockTime())
				vested = clawbackAccount.GetVestedCoins(ctx.BlockTime())
			...
			Context("before first vesting period - all tokens locked and unvested", func() {
				BeforeEach(func() {
					...
					Expect(unvested).To(Equal(lyfeblocutil.TestVestingSchedule.TotalVestingCoins))
			...
			Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
				BeforeEach(func() {
					...
					expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, lyfeblocutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(lyfeblocutil.TestVestingSchedule.CliffMonths))))
					Expect(vested).NotTo(Equal(lyfeblocutil.TestVestingSchedule.TotalVestingCoins))
