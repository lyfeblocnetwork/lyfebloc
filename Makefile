...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
LYFEBLOC_BINARY = lyfeblocd
LYFEBLOC_DIR = lyfebloc
BUILDDIR ?= $(CURDIR)/build
HTTPS_GIT := https://github.com/lyfeblocnetwork/lyfebloc.git
...
# Deps
DEPS_COSMOS_SDK_VERSION := $(shell cat go.sum | grep -E 'github.com/lyfebloc/cosmos-sdk\s' | grep -v -e 'go.mod' | tail -n 1 | awk '{ print $$2; }')
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=lyfebloc \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(LYFEBLOC_BINARY) \
...
build-docker-goleveldb:
	...
	mkdir -p ./build/.lyfeblocd
	echo '#!/usr/bin/env bash' > ./build/lyfeblocd
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/lyfeblocd
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/lyfeblocd
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.lyfeblocd:/home/lyfebloc/.lyfeblocd $$IMAGE_NAME lyfeblocd "$$@"' >> ./build/lyfeblocd
	chmod +x ./build/lyfeblocd
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/lyfeblocnetwork/lyfebloc"
	godoc -http=:6060
...
proto-download-deps:
	...
	git init && \
	git remote add origin "https://github.com/lyfebloc/cosmos-sdk.git" && \
...
PACKAGE_NAME:=github.com/lyfeblocnetwork/lyfebloc
