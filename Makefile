...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=lyfebloc-app \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=lyfeblocd \
...
## build: Build the lyfeblocd binary into the ./build directory.
build: mod
	@cd ./cmd/lyfeblocd
	@mkdir -p build/
	@go build $(BUILD_FLAGS) -o build/ ./cmd/lyfeblocd
.PHONY: build

## install: Build and install the lyfeblocd binary into the $GOPATH/bin directory.
install: go.sum
	@echo "--> Installing lyfeblocd"
	@go install $(BUILD_FLAGS) ./cmd/lyfeblocd
.PHONY: install
...
## build-docker: Build the lyfeblocd docker image. Requires docker.
build-docker:
	@echo "--> Building Docker image"
	$(DOCKER) build -t lyfeblocnetwork/lyfebloc-app -f Dockerfile .
.PHONY: build-docker
...
txsim-build-docker:
	docker build -t ghcr.io/lyfeblocnetwork/txsim -f docker/Dockerfile_txsim  .
...
adr-gen:
	...
	@curl -sSL https://raw.githubusercontent.com/lyfeblocnetwork/.github/main/adr-template.md > docs/architecture/adr-template.md
.PHONY: adr-gen
