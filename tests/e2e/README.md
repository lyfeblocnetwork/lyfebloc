...
for running end-to-end tests on Lyfebloc.
It is used for testing chain upgrades,
as it allows for initializing multiple Lyfebloc chains with different versions.
...
# version(s) of initial lyfebloc node(s) that will be upgraded, tag e.g. 'v9.1.0'
# to use multiple upgrades separate the versions with a forward slash, e.g.
# 'v10.0.1/v11.0.0-rc1'
INITIAL_VERSION

# version of upgraded lyfebloc node that will replace the initial node, tag e.g.
# 'v10.0.0'
TARGET_VERSION

# mount point for the upgraded node container, to mount new node version to
# previous node state folder. By default this is './build/.lyfeblocd:/root/.lyfeblocd'
# More info at https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' lyfebloc cli parameter, used to start nodes with a specific
# chain-id and submit proposals
# By default this is 'lyfebloc_1775-1'
CHAIN_ID
```
...
1. Build a docker image for the lyfebloc target version
...
the test suite exports `/.lyfeblocd` folder from the docker container
...
This package is decoupled from depending on the Lyfebloc codebase.
It initializes the chains for testing via Docker.  
As a result, the test suite may provide the
desired Lyfebloc version to Docker containers during the initialization.
This design allows for the opportunity of testing chain upgrades
by providing an older Lyfebloc version to the container,
...
to run different versions of lyfebloc containers,
...
will be pulled from [DockerHub](https://hub.docker.com/r/lyfeloop/lyfebloc/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `lyfebloc/app/upgrades` folder)
...
```log
ok  	github.com/lyfeblocnetwork/lyfebloc/v9/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        can't start lyfebloc node, container exit code: 2
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/lyfeblocnetwork/lyfebloc/v10/cmd/lyfeblocd/root.go:243 +0x2ca
        github.com/lyfebloc/ethermint/server.startInProcess(_, {{0x0, 0x0, 0x0},...
            github.com/lyfebloc/ethermint@v0.20.0-rc2/server/start.go:304 +0x9c5
        github.com/lyfebloc/ethermint/server.StartCmd.func2(0xc001620600?, {0xc0...
            github.com/lyfebloc/ethermint@v0.20.0-rc2/server/start.go:123 +0x1ec
        ...
        main.main()
            github.com/lyfeblocnetwork/lyfebloc/v10/cmd/lyfeblocd/main.go:20 +0x45
...
```log
CONTAINER ID   IMAGE
9307f5485323   lyfebloc:local    <-- upgraded node
f41c97d6ca21   lyfebloc:v9.0.0   <-- initial node
```
