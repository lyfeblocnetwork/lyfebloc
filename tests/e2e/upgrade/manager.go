...
import (
	...
	testnetwork "github.com/lyfeblocnetwork/lyfebloc/testutil/integration/lyfebloc/network"
...
func (m *Manager) BuildImage(name, version, dockerFile, contextDir string, args map[string]string) error {
	...
	opts := docker.BuildImageOptions{
		...
		NoCache: true,
		// name with tag, e.g. lyfebloc:v9.0.0
	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				stdOut, stdErr, _ := m.GetLogs(resource.Container.ID)
				return fmt.Errorf(
					"can't start lyfebloc node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					stdErr,
					stdOut,
				)
...
// WaitForHeight queries the Lyfebloc node every second until the node will reach the specified height.
...
// UnwrapBlockHeightPostV50 unwraps the block height from the output of the lyfeblocd query block command
...
// GetNodeVersion calls the Lyfebloc CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"lyfeblocd", "version"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return "", fmt.Errorf("lyfebloc version error: %s", errBuff.String())
	}
...
func (m *Manager) getTimeoutCommit(ctx context.Context) (*big.Int, error) {
	exec, err := m.CreateExec([]string{"grep", `\s*timeout_commit =`, "/root/.lyfeblocd/config/config.toml"}, m.ContainerID())
	if err != nil {
