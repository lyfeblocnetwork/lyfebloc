...
import (
	...
	"github.com/lyfeblocnetwork/lyfebloc/tests/integration/ledger/mocks"
			...
			Context("with cli command", func() {
				BeforeEach(func() {
					...
					err := testutil.FundAccount(
						...
						s.accAddr,
						sdk.NewCoins(
							sdk.NewCoin("alyfe", math.NewInt(100000000000000)),
						),
					)
					s.Require().NoError(err)
				...
				It("should execute bank tx cmd", func() {
					...
					cmd.SetArgs([]string{
						...
						sdk.NewCoin("alyfe", math.NewInt(1000)).String(),
						s.FormatFlag(flags.FlagUseLedger),
