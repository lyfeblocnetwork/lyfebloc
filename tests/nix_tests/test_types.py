...
def test_block(lyfebloc_cluster, geth):
    # Perform block queries (eth_getBlockByNumber & eth_getBlockByHash)
    # on lyfebloc and geth nodes, and compare the responses
    # Case 1: block without transactions
    get_blocks(lyfebloc_cluster, geth, False)
    # Case 2: block with transactions
    get_blocks(lyfebloc_cluster, geth, True)


def get_blocks(lyfebloc_rpc_ws, geth, with_transactions):
    w3: Web3 = lyfebloc_rpc_ws.w3
    eth_rpc = w3.provider
    ...
    lyfebloc_blk = w3.eth.get_block(1)
    # Get existing block, no transactions
    eth_rsp = eth_rpc.make_request(
        "eth_getBlockByHash", [lyfebloc_blk["hash"].hex(), with_transactions]
    )
...
def test_accounts(lyfebloc_cluster, geth):
...
def test_syncing(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_coinbase(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_max_priority_fee(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_gas_price(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_block_number(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_balance(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_storage_at(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_proof(lyfebloc_cluster, geth):
    # on lyfebloc the proof query will fail for block numbers <= 2
...
def test_get_code(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_block_transaction_count(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_transaction(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_transaction_receipt(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_fee_history(lyfebloc_cluster, geth):
    w3: Web3 = lyfebloc_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_estimate_gas(lyfebloc_cluster, geth):
...
def test_incomplete_send_transaction(lyfebloc_cluster, geth):
