...
from .network import (
    CosmosChain,
    Hermes,
    build_patched_lyfeblocd,
    create_snapshots_dir,
    setup_custom_lyfebloc,
)
from .utils import (
    ADDRS,
    eth_to_bech32,
    ...
    setup_stride,
    update_lyfebloc_bin,
    update_lyfeblocd_and_setup_stride,
    wait_for_fn,
    wait_for_port,
)

# alyfe IBC representation on another chain connected via channel-0.
LYFEBLOC_IBC_DENOM = "ibc/8EAC8061F4499F03D2D1419A3E73D346289AE9DB89CAB1486B72539572B1915E"
# uosmo IBC representation on the Lyfebloc chain.
...
# uatom from cosmoshub-1 IBC representation on the Lyfebloc chain and on Cosmos Hub 2 chain.
...
IBC_CHAINS_META = {
    "lyfebloc": {
        "chain_name": "lyfebloc_1775-1",
        "bin": "lyfeblocd",
        "denom": "alyfe",
    },
    "lyfebloc-rocksdb": {
        "chain_name": "lyfebloc_1775-1",
        "bin": "lyfeblocd-rocksdb",
        "denom": "alyfe",
    },
    "chainmain": {
...
EVM_CHAINS = ["lyfebloc_1775", "chainmain-1"]
...
def get_lyfebloc_generator(
    ...
    """
    setup lyfebloc with custom config
    depending on the build
    """
    post_init_func = None
    if is_rocksdb:
        file = memiavl_config(tmp_path, file)
        gen = setup_custom_lyfebloc(
            tmp_path,
            26710,
        ...
        if custom_scenario:
            ...
            modified_bin = build_patched_lyfeblocd(custom_scenario)
            post_init_func = update_lyfebloc_bin(modified_bin)
            if stride_included:
                post_init_func = update_lyfeblocd_and_setup_stride(modified_bin)
            gen = setup_custom_lyfebloc(
                tmp_path,
                26700,
                Path(__file__).parent / file,
...
def prepare_network(
    ...
    for chain in chain_names:
        ...
        # lyfebloc is the first chain
        # set it up and the relayer
        if "lyfebloc" in chain_name:
            # setup lyfebloc with the custom config
            # depending on the build
            gen = get_lyfebloc_generator(
                tmp_path,
                file,
                "-rocksdb" in chain,
            ...
            lyfebloc = next(gen)  # pylint: disable=stop-iteration-return

            # setup relayer
            hermes = Hermes(tmp_path / "relayer.toml")

            # wait for grpc ready
            wait_for_port(ports.grpc_port(lyfebloc.base_port(0)))  # lyfebloc grpc
            chains["lyfebloc"] = lyfebloc
            continue
    ...
    lyfebloc.supervisorctl("start", "relayer-demo")
...
def hermes_transfer(ibc, other_chain_name="chainmain-1", other_chain_denom="basecro"):
    assert_ready(ibc)
    # chainmain-1 -> lyfebloc_1775-1
    my_ibc0 = other_chain_name
    my_ibc1 = "lyfebloc_1775-1"
...
def setup_denom_trace(ibc):
    """
    Helper setup function to send some funds from chain-main to lyfebloc
    to register the denom trace (if not registered already)
    """
    res = ibc.chains["lyfebloc"].cosmos_cli().denom_traces()
    if len(res["denom_traces"]) == 0:
        amt = 100
        src_denom = "basecro"
        dst_addr = ibc.chains["lyfebloc"].cosmos_cli().address("signer2")
