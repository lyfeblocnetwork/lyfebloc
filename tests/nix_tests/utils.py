...
LYFEBLOC_ADDRESS_PREFIX = "lyfebloc"
DEFAULT_DENOM = "alyfe"
WLYFEBLOC_ADDRESS = Web3.toChecksumAddress("0xD4949664cD82660AaE99bEdc034a0deA8A0bd517")
TEST_CONTRACTS = {
    ...
    "InterchainSender": "lyfebloc/testutil/contracts/InterchainSender.sol",
    "InterchainSenderCaller": "lyfebloc/testutil/contracts/InterchainSenderCaller.sol",
    "ICS20I": "lyfebloc/ics20/ICS20I.sol",
    "DistributionI": "lyfebloc/distribution/DistributionI.sol",
    "DistributionCaller": "lyfebloc/testutil/contracts/DistributionCaller.sol",
    "StakingI": "lyfebloc/staking/StakingI.sol",
    "StakingCaller": "lyfebloc/staking/testdata/StakingCaller.sol",
    "IStrideOutpost": "lyfebloc/outposts/stride/IStrideOutpost.sol",
    "IOsmosisOutpost": "lyfebloc/outposts/osmosis/IOsmosisOutpost.sol",
    "IERC20": "lyfebloc/erc20/IERC20.sol",
}

OSMOSIS_POOLS = {
    "Lyfebloc_Osmo": Path(__file__).parent / "osmosis/lyfeblocOsmosisPool.json",
}
...
REGISTER_ERC20_PROP = {
    "messages": [
        {
            "@type": "/lyfebloc.erc20.v1.MsgRegisterERC20",
...
def eth_to_bech32(addr, prefix=LYFEBLOC_ADDRESS_PREFIX):
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:lyfebloc_1775-1-node{i}":
            ini[section].update(
                {
...
def update_lyfeblocd_and_setup_stride(modified_bin):
    def inner(path, base_port, config):  # pylint: disable=unused-argument
        update_lyfebloc_bin(modified_bin)(path, base_port, config)
        setup_stride()(path, base_port, config)

    return inner


def update_lyfebloc_bin(
    modified_bin, nodes=[0, 1]
):  # pylint: disable=dangerous-default-value
    """
    updates the lyfebloc binary with a patched binary.
...
def debug_trace_tx(lyfebloc, tx_hash: str):
    url = f"http://127.0.0.1:{ports.evmrpc_port(lyfebloc.base_port(0))}"
    params = {
