...
# The token factory IBC denom on Lyfebloc
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [lyfebloc_build, "osmosis"])
    yield from network
...
def test_str_v2_token_factory(ibc):
    ...
    balance = get_balance(lyfebloc, bech_dst, TOKEN_FACTORY_IBC_DENOM)
    assert balance == 100
