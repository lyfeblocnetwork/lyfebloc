...
@pytest.fixture(scope="module", params=["lyfebloc"])
...
def test_denom_trace(ibc, name, args, err_contains, exp_res):
    ...
    # setup: send some funds from chain-main to lyfebloc
    ...
    try:
        # make the query call
        pc = get_precompile_contract(ibc.chains["lyfebloc"].w3, "ICS20I")
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                ADDRS["community"],
                [["transfer", "channel-0", [["alyfe", int(1e18)]], [], []]],
            ],
            [],
            "improper number of arguments",
            None,
        ...
        (
            ...
            None,
            [("transfer", "channel-0", [("alyfe", 1000000000000000000)], [], [])],
        ),
        (
            ...
            [
                (
                    "transfer",
                    "channel-0",
                    [("alyfe", 1000000000000000000), ("uatom", 1000000000000000000)],
                    [],
                    [],
                )
            ],
...
def test_query_allowance(ibc, name, auth_args, args, err_contains, exp_res):
    ...
    gas_limit = 200_000
    pc = get_precompile_contract(ibc.chains["lyfebloc"].w3, "ICS20I")
    lyfebloc_gas_price = ibc.chains["lyfebloc"].w3.eth.gas_price
    ...
    tx_receipt = send_transaction(ibc.chains["lyfebloc"].w3, approve_tx, KEYS["validator"])
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-1", [["alyfe", int(1e18)]], [], []]],
            ],
            True,
            "channel not found",
            0,
        ...
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-0", [["alyfe", MAX_UINT256]], [], []]],
            ],
            False,
            "",
            MAX_UINT256,
...
def test_approve(ibc, name, args, exp_err, err_contains, exp_spend_limit):
    ...
    tx_receipt = send_transaction(ibc.chains["lyfebloc"].w3, approve_tx, KEYS["signer1"])
    if exp_err:
        assert tx_receipt.status == 0, f"Failed: {name}"
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["lyfebloc"], tx_receipt.transactionHash.hex())
    ...
    assert auth_event.args.allocations[0] == (
        "transfer",
        "channel-0",
        [("alyfe", exp_spend_limit)],
        [],
        [],
    )

    # check the authorization was created
    cli = ibc.chains["lyfebloc"].cosmos_cli()
...
def test_revoke(ibc, name, args, exp_err, err_contains):
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["lyfebloc"].w3, revoke_tx, KEYS["signer1"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "increase allowance by 1 LYFEBLOC",
            [
                ADDRS["signer2"],
                [["transfer", "channel-0", [["alyfe", int(1e18)]], [], []]],
            ],
            ...
            False,
            "",
            json.loads("""[{"denom": "alyfe", "amount": "2000000000000000000"}]"""),
        ),
        (
            ...
            "",
            json.loads(
                """[{"denom": "alyfe", "amount": "1000000000000000000"},"""
...
def test_increase_allowance(
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["lyfebloc"].w3, incr_allowance_tx, KEYS["validator"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "decrease allowance by 0.5 LYFEBLOC",
            [
                ADDRS["signer2"],
                [["transfer", "channel-0", [["alyfe", int(1e18)]], [], []]],
            ],
            ...
            False,
            "",
            json.loads("""[{"denom": "alyfe", "amount": "500000000000000000"}]"""),
        ),
        (
...
def test_decrease_allowance(
    ...
    tx_receipt = send_transaction(ibc.chains["lyfebloc"].w3, approve_tx, KEYS["community"])
    assert tx_receipt.status == 1
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["lyfebloc"].w3, decr_allowance_tx, KEYS["community"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "transfer 1 Lyfebloc from chainA to chainB and spend the entire allowance",
            [["alyfe", int(1e18)]],
            [
                "transfer",
                "channel-0",
        ...
        (
            "transfer 1 Lyfebloc from chainA to chainB and don't change the unlimited spending limit",
            [["alyfe", MAX_UINT256]],
            [
                "transfer",
                "channel-0",
            ...
            None,
            int(1e18),
            json.loads(f"""[{{"denom": "alyfe", "amount": "{MAX_UINT256}"}}]"""),
        ),
        (
            "transfer 1 Lyfebloc from chainA to chainB and only change 1 spend limit",
            [["alyfe", int(1e18)], ["uatom", int(1e18)]],
            [
                "transfer",
                "channel-0",
...
def test_ibc_transfer_with_authorization(
    ...
    if auth_coins is not None:
        ...
        receipt = send_transaction(ibc.chains["lyfebloc"].w3, approve_tx, KEYS["signer2"])
    ...
    src_amount_lyfebloc_prev = get_balance(ibc.chains["lyfebloc"], src_address, src_denom)
    dst_balance_prev = get_balance(
        ibc.chains["chainmain"], dst_address, LYFEBLOC_IBC_DENOM
    )
    try:
        ...
        receipt = send_transaction(ibc.chains["lyfebloc"].w3, transfer_tx, KEYS["signer2"])
    except Exception as err:
        check_error(err, err_contains)
        return
    ...
    if exp_err:
        ...
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["lyfebloc"], receipt.transactionHash.hex())
    ...
    assert receipt.status == 1, debug_trace_tx(
        ibc.chains["lyfebloc"], receipt.transactionHash.hex()
    )
    fees = receipt.gasUsed * lyfebloc_gas_price
    ...
    assert transfer_event.args.denom == "alyfe"
    ...
    src_amount_lyfebloc_final = get_balance(ibc.chains["lyfebloc"], src_address, src_denom)

    assert src_amount_lyfebloc_final == src_amount_lyfebloc_prev - fees - transfer_amt
...
def test_ibc_transfer_from_eoa_through_contract(ibc):
    ...
    src_adr = ibc.chains["lyfebloc"].cosmos_cli().address("signer2")
    ...
    tx_receipt = send_transaction(ibc.chains["lyfebloc"].w3, approve_tx, KEYS["signer2"])
    assert tx_receipt.status == 1
    ...
    src_starting_balance = get_balance(ibc.chains["lyfebloc"], src_adr, "alyfe")
    ...
    send_tx = eth_contract.functions.transferFromEOA(
        ...
        {"from": ADDRS["signer2"], "gasPrice": lyfebloc_gas_price, "gas": gas_limit}
    )
    receipt = send_transaction(ibc.chains["lyfebloc"].w3, send_tx, KEYS["signer2"])
    ...
    src_final_amount_lyfebloc = get_balance(ibc.chains["lyfebloc"], src_adr, src_denom)
    assert src_final_amount_lyfebloc == src_starting_balance - amt - fees
...
def test_ibc_transfer_from_eoa_with_internal_transfer(
    ...
    src_denom = "alyfe"
    gas_limit = 200_000
    lyfebloc_gas_price = w3.eth.gas_price

    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    # address that will escrow the alyfe tokens when transferring via IBC
    escrow_bech32 = lyfebloc.cosmos_cli().escrow_address(channel)
    ...
    eth_contract = setup_interchain_sender_contract(
        lyfebloc, ACCOUNTS["signer2"], amt, initial_contract_balance
    )

    # get starting balances to check after the tx
    src_bech32 = lyfebloc.cosmos_cli().address("signer2")
    contract_bech32 = eth_to_bech32(eth_contract.address)
    other_addr_bech32 = eth_to_bech32(other_addr)

    src_starting_balance = get_balance(ibc.chains["lyfebloc"], src_bech32, "alyfe")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, LYFEBLOC_IBC_DENOM
    )
    other_addr_initial_balance = get_balance(
        ibc.chains["lyfebloc"], other_addr_bech32, src_denom
    )
    escrow_initial_balance = get_balance(ibc.chains["lyfebloc"], escrow_bech32, src_denom)
    ...
    if other_addr == src_addr:
        ...
        other_addr_final_balance = get_balance(
            ibc.chains["lyfebloc"], escrow_bech32, src_denom
        )
    ...
    src_final_amount_lyfebloc = get_balance(ibc.chains["lyfebloc"], src_bech32, src_denom)
    assert src_final_amount_lyfebloc == exp_src_final_bal, f"Failed {name}"

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["lyfebloc"], contract_bech32, src_denom
    )
    assert (
    ...
    escrow_final_balance = get_balance(ibc.chains["lyfebloc"], escrow_bech32, src_denom)
...
def test_ibc_multi_transfer_from_eoa_with_internal_transfer(
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = lyfebloc.cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    eth_contract = setup_interchain_sender_contract(
        lyfebloc, ACCOUNTS["signer2"], ibc_transfer_amt, initial_contract_balance
    )

    # send some funds (100 alyfe) to the contract to perform
    # internal transfer within the tx
    src_bech32 = eth_to_bech32(src_addr)
    contract_bech32 = eth_to_bech32(eth_contract.address)

    # get starting balances to check after the tx
    src_starting_balance = get_balance(lyfebloc, src_bech32, "alyfe")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, LYFEBLOC_IBC_DENOM
    )
    escrow_initial_balance = get_balance(lyfebloc, escrow_bech32, src_denom)
...
def test_multi_ibc_transfers_with_revert(ibc):
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = ibc.chains["lyfebloc"].cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    interchain_sender_contract = setup_interchain_sender_contract(
        lyfebloc, ACCOUNTS["signer2"], ibc_transfer_amt * 2, initial_contract_balance
    )
    ...
    assert src_final_amount_lyfebloc == exp_src_final_bal

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["lyfebloc"], interchain_sender_contract_bech32, src_denom
    )
    assert (
        contract_final_balance
        == initial_contract_balance - exp_amt_tranferred_internally
...
def setup_interchain_sender_contract(
    lyfebloc, src_acc, transfer_amt, initial_contract_balance
):
    """
    ...
    src_denom = "alyfe"
    gas_limit = 200_000
    lyfebloc_gas_price = lyfebloc.w3.eth.gas_price
    # Deployment of contracts and initial checks
    eth_contract, tx_receipt = deploy_contract(lyfebloc.w3, CONTRACTS["InterchainSender"])
    assert tx_receipt.status == 1

    counter = eth_contract.functions.counter().call()
    assert counter == 0

    pc = get_precompile_contract(lyfebloc.w3, "ICS20I")
    # Approve the contract to spend the src_denom
    approve_tx = pc.functions.approve(
    ...
    tx_receipt = send_transaction(lyfebloc.w3, approve_tx, src_acc.key)
    assert tx_receipt.status == 1
    ...
    contract_bech32 = eth_to_bech32(eth_contract.address)
    fund_tx = lyfebloc.cosmos_cli().transfer(
        src_bech32,
        contract_bech32,
        f"{initial_contract_balance}alyfe",
        gas_prices=f"{lyfebloc_gas_price + 100000}alyfe",
        generate_only=True,
    )

    fund_tx = lyfebloc.cosmos_cli().sign_tx_json(fund_tx, src_bech32, max_priority_price=0)
    rsp = lyfebloc.cosmos_cli().broadcast_tx_json(fund_tx, broadcast_mode="sync")
    assert rsp["code"] == 0, rsp["raw_log"]
    txhash = rsp["txhash"]
    wait_for_new_blocks(lyfebloc.cosmos_cli(), 2)
    receipt = lyfebloc.cosmos_cli().tx_search_rpc(f"tx.hash='{txhash}'")[0]
