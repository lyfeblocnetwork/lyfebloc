...
def custom_lyfebloc(tmp_path_factory):
    ...
    yield from setup_lyfebloc(path, 26800, long_timeout_commit=True)
...
def custom_lyfebloc_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_lyfebloc_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_priority(lyfebloc_cluster):
    """
...
def test_native_tx_priority(lyfebloc_cluster):
    ...
    test_cases = [
        {
            ...
            "amount": "1000alyfe",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 600000}alyfe",
        ...
        },
        {
            ...
            "amount": "1000alyfe",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 400000}alyfe",
    ...
    for tc in test_cases:
        ...
        gas_price = int(tc["gas_prices"].removesuffix("alyfe"))
