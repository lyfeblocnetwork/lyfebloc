...
from .network import setup_custom_lyfebloc, setup_lyfebloc
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_lyfebloc(tmp_path_factory):
    ...
    yield from setup_lyfebloc(path, 26200)


@pytest.fixture(scope="module")
def lyfebloc_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module",
    params=["lyfebloc", "geth", "lyfebloc-ws", "lyfebloc-rocksdb", "enable-indexer"],
)
def cluster(request, custom_lyfebloc, lyfebloc_rocksdb, lyfebloc_indexer, geth):
    """
    run on both lyfebloc and geth
    """
