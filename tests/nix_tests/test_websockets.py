def test_single_request_netversion(lyfebloc_cluster):
    lyfebloc_cluster.use_websocket()
    eth_ws = lyfebloc_cluster.w3.provider
...
def test_batch_request_netversion(lyfebloc):
    return


def test_ws_subscribe_log(lyfebloc):
    return


def test_ws_subscribe_newheads(lyfebloc):
    return
