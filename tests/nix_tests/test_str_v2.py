...
# uatom from cosmoshub-2 -> cosmoshub-1 IBC representation on the Lyfebloc chain.
...
# The ERC20 address of ATOM on Lyfebloc
...
UPDATE_PARAMS_PROP = {
    "messages": [
        {
            "@type": "/lyfebloc.erc20.v1.MsgUpdateParams",
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [lyfebloc_build, "cosmoshub-1", "cosmoshub-2"])
    yield from network
...
def test_str_v2_single_hop(ibc):
    ...
    w3 = lyfebloc.w3
    lyfebloc_cli = lyfebloc.cosmos_cli()
    lyfebloc_addr = ADDRS["signer2"]
    bech_dst = eth_to_bech32(lyfebloc_addr)

    gaia_cli = gaia.cosmos_cli()
    gaia_addr = gaia_cli.address("signer2")

    # Before IBC transfer, check no dynamic precompiles available
    active_dynamic_precompiles = lyfebloc_cli.erc20_params()["params"][
        "dynamic_precompiles"
    ]
    assert len(active_dynamic_precompiles) == 0

    # Check token pairs before IBC transfer,
    # should only exist the WLYFEBLOC pair
    pairs = lyfebloc_cli.get_token_pairs()
    assert len(pairs) == 1

    old_dst_balance = get_balance(lyfebloc, bech_dst, ATOM_IBC_DENOM)
    rsp = gaia_cli.ibc_transfer(
        gaia_addr, bech_dst, "5000uatom", "channel-0", 1, fees="10000uatom"
    )
    assert rsp["code"] == 0

    wait_for_ack(lyfebloc_cli, "Lyfebloc")

    pairs = lyfebloc_cli.get_token_pairs()
    active_dynamic_precompiles = lyfebloc_cli.erc20_params()["params"][
        "dynamic_precompiles"
    ]
    new_dest_balance = get_balance(lyfebloc, bech_dst, ATOM_IBC_DENOM)
    erc_dest_balance = erc20_balance(w3, ATOM_1_ERC20_ADDRESS, lyfebloc_addr)
...
def test_str_v2_multi_hop(ibc):
    ...
    lyfebloc_old_balance = get_balance(lyfebloc, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    ...
    lyfebloc_balance = get_balance(lyfebloc, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    dynamic_precompiles = lyfebloc_cli.erc20_params()["params"]["dynamic_precompiles"]
    token_pairs = lyfebloc_cli.get_token_pairs()

    # Here it's only one from the previous one we've registered in the first test
    assert lyfebloc_old_balance + 50000 == lyfebloc_balance
    assert len(dynamic_precompiles) == 1
    assert dynamic_precompiles[0] == ATOM_1_ERC20_ADDRESS
    assert len(token_pairs) == 2


def test_wlyfebloc_precompile_transfer(ibc):
    """
    Test the ERC20 transfer from one signer to another using the now
    registered ERC20 precompiled contract for WLYFEBLOC.
    """
    assert_ready(ibc)
    ...
    w3 = lyfebloc.w3
    lyfebloc_balance = get_balance(lyfebloc, bech_dst, src_denom)
    signer2_balance = erc20_balance(w3, WLYFEBLOC_ADDRESS, signer2)

    assert lyfebloc_balance == signer2_balance

    receipt = erc20_transfer(
        w3, WLYFEBLOC_ADDRESS, signer1, signer2, 1000000, KEYS["signer1"]
    )
    assert receipt.status == 1

    signer_2_balance_after = erc20_balance(w3, WLYFEBLOC_ADDRESS, signer2)
    assert signer_2_balance_after == signer2_balance + 1000000

    lyfebloc_balance_after = get_balance(lyfebloc, bech_dst, src_denom)
    assert lyfebloc_balance_after == lyfebloc_balance + 1000000
...
def test_toggle_erc20_precompile(ibc):
    ...
    acc = lyfebloc_cli.evm_account(contract_bech32_addr)
    ...
    initial_params = lyfebloc_cli.erc20_params()

    # update params via gov proposal to disable all the erc20 precompile
    update_erc20_params(lyfebloc)
...
def update_erc20_params(lyfebloc: Lyfebloc, native_precomiles=[], dynamic_precompiles=[]):
