...
def test_ibc_callbacks(
    ...
    bech32_lyfebloc_addr = lyfebloc_cli.address("signer2")
    ...
    with tempfile.NamedTemporaryFile("w") as fp:
        ...
        fp.flush()
        rsp = lyfebloc_cli.gov_proposal("signer2", fp.name)
        assert rsp["code"] == 0, rsp["raw_log"]
        txhash = rsp["txhash"]

        wait_for_new_blocks(lyfebloc_cli, 2)
        receipt = lyfebloc_cli.tx_search_rpc(f"tx.hash='{txhash}'")[0]
        assert receipt["tx_result"]["code"] == 0, rsp["raw_log"]

    res = lyfebloc_cli.query_proposals()
    props = res["proposals"]
    ...
    erc20_balance = contract.functions.balanceOf(lyfebloc_addr).call()
    assert erc20_balance == initial_amt

    # convert to IBC voucher
    ibc_voucher_denom = f"erc20/{contract.address}"
    if convert_amt > 0:
        rsp = lyfebloc_cli.convert_erc20(contract.address, convert_amt, "signer2")
        assert rsp["code"] == 0, rsp["raw_log"]
        wait_for_new_blocks(lyfebloc_cli, 2)
    ...
    ibc_voucher_balance = get_balance(lyfebloc, bech32_lyfebloc_addr, ibc_voucher_denom)
    assert ibc_voucher_balance == convert_amt

    # send erc20 via IBC
    rsp = lyfebloc_cli.ibc_transfer(
        bech32_lyfebloc_addr,
        dst_addr,
        f"{transfer_amt}{ibc_voucher_denom}",
        "channel-0",
        1,
        1,
        fees=f"{int(1e17)}alyfe",
    )
    assert rsp["code"] == 0, rsp["raw_log"]
    wait_for_new_blocks(lyfebloc_cli, 2)
