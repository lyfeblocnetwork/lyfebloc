...
def test_ibc_transfer_with_hermes(ibc):
    ...
    old_dst_balance = get_balance(ibc.chains["lyfebloc"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["lyfebloc"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("alyfe"))
...
def test_lyfebloc_ibc_transfer(ibc):
    """
...
def test_lyfebloc_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending alyfe from lyfebloc to crypto-org-chain using cli
