...
DEFAULT_CHAIN_BINARY = "lyfeblocd"
...
def setup_lyfebloc(path, base_port, long_timeout_commit=False):
    ...
    yield from setup_custom_lyfebloc(path, base_port, cfg)
...
def create_snapshots_dir(
    ...
    for idx in range(n_nodes):
        data_snapshots_dir = path / "lyfebloc_1775-1" / f"node{idx}" / "data" / "snapshots"
        os.makedirs(data_snapshots_dir, exist_ok=True)


def setup_lyfebloc_rocksdb(path, base_port, long_timeout_commit=False):
    """
    setup_lyfebloc_rocksdb returns an Lyfebloc chain compiled with RocksDB
...
def setup_custom_lyfebloc(
    ...
    try:
        ...
        yield Lyfebloc(
            path / "lyfebloc_1775-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
...
def build_patched_lyfeblocd(patch_nix_file):
    """
    build the binary modified for a custom scenario
    e.g. allow to register WLYFEBLOC token
