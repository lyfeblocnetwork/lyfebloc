...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print lyfeblocd output, default false').argv
...
function setupNetwork ({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const serverStartedMsg = 'lyfeblocd started'

    const lyfeblocdProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting lyfeblocd process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      lyfeblocdProc.stdout.pipe(process.stdout)
    }

    lyfeblocdProc.stdout.on('data', (d) => {
      ...
      if (oLine.indexOf(serverStartedLog) !== -1) {
        ...
        resolve(lyfeblocdProc)
      }
    })

    lyfeblocdProc.stderr.on('data', (d) => {
      const oLine = d.toString()
      if (runConfig.verboseLog) {
        process.stdout.write(oLine)
      }
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start lyfeblocd timeout!')), timeout)
  })
