...
import (
	...
	"github.com/lyfeblocnetwork/lyfebloc/crypto/hd"
	"github.com/lyfeblocnetwork/lyfebloc/server/config"
	...
	"github.com/lyfeblocnetwork/lyfebloc/testutil/network"
)
...
Example:
	lyfeblocd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
...
Example:
	lyfeblocd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, lyfebloctypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(lyfebloctypes.AttoLyfebloc, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, &lyfebloctypes.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(addr, nil, 0, 0),
			CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
		})

		valTokens := sdk.TokensFromConsensusPower(100, lyfebloctypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			addr.String(),
			valPubKeys[i],
			sdk.NewCoin(lyfebloctypes.AttoLyfebloc, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(lyfebloctypes.AttoLyfebloc)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, lyfebloctypes.AttoLyfebloc, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
