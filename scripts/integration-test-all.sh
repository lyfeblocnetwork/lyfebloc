...
## default port prefixes for lyfeblocd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
	"$PWD"/build/lyfeblocd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
	"$PWD"/build/lyfeblocd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
	"$PWD"/build/lyfeblocd add-genesis-account \
		"$("$PWD"/build/lyfeblocd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000alyfe,1000000000000000000stake \
		--keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/lyfeblocd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/lyfeblocd collect-gentxs --home "$DATA_DIR$i"
	"$PWD"/build/lyfeblocd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
	echo "starting lyfebloc node $i in background ..."
	"$PWD"/build/lyfeblocd start --pruning=nothing --rpc.unsafe \
	...
	LYFEBLOC_PID=$!
	echo "started lyfebloc node, pid=$LYFEBLOC_PID"
	# add PID to array
	arr+=("$LYFEBLOC_PID")
...
if [[ -z $TEST || $TEST == "rpc" || $TEST == "pending" ]]; then
	...
	for i in $(seq 1 "$TEST_QTD"); do
		...
		echo "going to test lyfebloc node $HOST_RPC ..."
...
stop_func() {
	LYFEBLOC_PID=$i
	echo "shutting down node, pid=$LYFEBLOC_PID ..."

	# Shutdown lyfebloc node
	kill -9 "$LYFEBLOC_PID"
	wait "$LYFEBLOC_PID"
