...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdkmath.NewIntFromUint64(uint64(feeAmt))) // 0.01 LYFEBLOC
)
...
func signCosmosTx(
	...
	seq, err := appLyfebloc.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appLyfebloc.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
