...
func PrepareEthTx(
	...
	signer := ethtypes.LatestSignerForChainID(appLyfebloc.EvmKeeper.ChainID())
	txFee := sdk.Coins{}
	txGasLimit := uint64(0)
...
func CreateEthTx(
	...
	chainID := appLyfebloc.EvmKeeper.ChainID()

	// When we send multiple Ethereum Tx's in one Cosmos Tx, we need to increment the nonce for each one.
	nonce := appLyfebloc.EvmKeeper.GetNonce(ctx, fromAddr) + uint64(nonceIncrement) //nolint:gosec // G115
	evmTxParams := &evmtypes.EvmTxArgs{
		...
		Amount:    amount,
		GasLimit:  100000,
		GasFeeCap: appLyfebloc.FeeMarketKeeper.GetBaseFee(ctx),
