...
import (
	...
	"github.com/lyfeblocnetwork/lyfebloc/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Lyfebloc, t time.Duration, vs *cmttypes.ValidatorSet) (sdk.Context, error) {
...
func CommitAndCreateNewCtx(ctx sdk.Context, app *app.Lyfebloc, t time.Duration, vs *cmttypes.ValidatorSet) (sdk.Context, error) {
	header, err := commit(ctx, app, t, vs)
	if err != nil {
		return ctx, err
	}
...
func DeliverTx(
	ctx sdk.Context,
	appLyfebloc *app.Lyfebloc,
	priv cryptotypes.PrivKey,
	gasPrice *sdkmath.Int,
	msgs ...sdk.Msg,
) (abci.ExecTxResult, error) {
	txConfig := appLyfebloc.GetTxConfig()
	...
	return BroadcastTxBytes(appLyfebloc, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appLyfebloc, priv, msgs...)
	if err != nil {
		return abci.ExecTxResult{}, err
	}
	res, err := BroadcastTxBytes(appLyfebloc, txConfig.TxEncoder(), tx)
	if err != nil {
		return res, err
	}

	codec := appLyfebloc.AppCodec()
...
func CheckTx(
	...
	return checkTxBytes(appLyfebloc, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Lyfebloc, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ExecTxResult, error) {
...
func commit(ctx sdk.Context, app *app.Lyfebloc, t time.Duration, vs *cmttypes.ValidatorSet) (tmproto.Header, error) {
...
func checkTxBytes(app *app.Lyfebloc, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
