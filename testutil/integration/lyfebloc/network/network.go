...
import (
	...
	commonnetwork "github.com/lyfeblocnetwork/lyfebloc/testutil/integration/common/network"
...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	lyfeblocApp := createLyfeblocApp(n.cfg.chainID, n.cfg.customBaseAppOpts...)
	...
	slashingParams, err := getValidatorsSlashingGen(validators, lyfeblocApp.StakingKeeper)
	if err != nil {
		return err
	}
	...
	genesisState, err = customizeGenesis(lyfeblocApp, n.cfg.customGenesisState, genesisState)
	if err != nil {
		return err
	}
	...
	now := time.Now().UTC()
	if _, err := lyfeblocApp.InitChain(
	...
	header := cmtproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             lyfeblocApp.LastBlockHeight() + 1,
		AppHash:            lyfeblocApp.LastCommitID().Hash,
		Time:               now,
	...
	if _, err := lyfeblocApp.FinalizeBlock(req); err != nil {
		return err
	}

	// TODO - this might not be the best way to initilize the context
	n.ctx = lyfeblocApp.BaseApp.NewContextLegacy(false, header)

	// Commit genesis changes
	if _, err := lyfeblocApp.Commit(); err != nil {
		return err
	}
