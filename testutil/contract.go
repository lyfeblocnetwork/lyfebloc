...
import (
	...
	evm "github.com/lyfeblocnetwork/lyfebloc/x/evm/types"
)
...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the alyfe amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	ctx sdk.Context,
	lyfeblocApp *app.Lyfebloc,
	priv cryptotypes.PrivKey,
	queryClientEvm evm.QueryClient,
	contract evm.CompiledContract,
	constructorArgs ...interface{},
) (common.Address, error) {
	chainID := lyfeblocApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := lyfeblocApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: lyfeblocApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(lyfeblocApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, lyfeblocApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := lyfeblocApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := lyfeblocApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := lyfeblocApp.EvmKeeper.GetNonce(ctx, from)
