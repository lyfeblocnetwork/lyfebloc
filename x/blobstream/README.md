...
The Blobstream query attestation command is part of the `lyfeblocd` binary. It allows the user to query specific attestations by their corresponding nonce.

```shell
$ lyfeblocd query blobstream attestation --help
query an attestation by nonce

Usage:
  lyfeblocd query blobstream attestation <nonce> [flags]
...
The Blobstream verification command is part of the `lyfeblocd` binary. It allows the user to verify that a set of shares has been posted to a specific Blobstream contract.

```shell
$ lyfeblocd verify --help
...
Use "lyfeblocd verify [command] --help" for more information about a command.
```
