...
var (
	...
	setTotalBonded = func(s tallyFixture, n int64) {
		s.mocks.AcctKeeper.EXPECT().AddressCodec().Return(address.NewBech32Codec("lyfebloc")).AnyTimes()
		s.mocks.StakingKeeper.EXPECT().ValidatorAddressCodec().Return(address.NewBech32Codec("lyfeblocvaloper")).AnyTimes()
