...
import (
	...
	"github.com/lyfeblocnetwork/lyfebloc/x/evm/keeper"
...
func (suite *KeeperTestSuite) TestWithChainID() {
	testCases := []struct {
		...
		{
			"success - Lyfebloc mainnet chain ID",
			"lyfebloc_1776-2",
			9001,
			false,
		},
		{
			"success - Lyfebloc testnet chain ID",
			"lyfebloc_1775-4",
			9000,
			false,
		},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.network.App.AccountKeeper.IterateAccounts(suite.network.GetContext(), func(account sdk.AccountI) bool {
				ethAccount, ok := account.(lyfebloctypes.EthAccountI)
				if !ok {
