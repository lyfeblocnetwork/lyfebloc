...
func TestQueryCirculatingSupply(t *testing.T) {
	...
	mintCoin := sdk.NewCoin(mintDenom, sdk.TokensFromConsensusPower(int64(400_000_000), lyfebloctypes.PowerReduction))
	err := nw.App.InflationKeeper.MintCoins(ctx, mintCoin)
	require.NoError(t, err)

	// team allocation is zero if not on mainnet
	expCirculatingSupply := sdk.NewDecCoin(mintDenom, sdk.TokensFromConsensusPower(200_000_000, lyfebloctypes.PowerReduction))
...
func TestQueryInflationRate(t *testing.T) {
	...
	// Mint coins to increase supply
	mintDenom := nw.App.InflationKeeper.GetParams(ctx).MintDenom
	mintCoin := sdk.NewCoin(mintDenom, sdk.TokensFromConsensusPower(int64(400_000_000), lyfebloctypes.PowerReduction).Sub(bondedAmt))
	err := nw.App.InflationKeeper.MintCoins(ctx, mintCoin)
	require.NoError(t, err)
