...
func CalculateEpochMintProvision(
	...
	// Multiply epochMintProvision with power reduction (10^18 for lyfebloc) as the
	// calculation is based on `lyfebloc` and the issued tokens need to be given in
	// `alyfe`
	epochProvision = epochProvision.Mul(math.LegacyNewDecFromInt(lyfebloctypes.PowerReduction))
