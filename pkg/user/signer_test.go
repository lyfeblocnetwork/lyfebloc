...
import (
	...
	util "github.com/lyfeblocnetwork/lyfebloc/test/util"
	"github.com/lyfeblocnetwork/lyfebloc/test/util/blobfactory"
...
func (s *SignerTestSuite) TestGasConsumption() {
	...
	ublocToSend := int64(1)
	msg := bank.NewMsgSend(s.signer.Address(), testnode.RandomAddress().(sdk.AccAddress), sdk.NewCoins(sdk.NewInt64Coin(appconsts.BondDenom, ublocToSend)))

	gasPrice := int64(1)
	gasLimit := uint64(1e6)
	fee := uint64(1e6) // 1 TIA
	// Note: gas price * gas limit = fee amount. So by setting gasLimit and fee
	// to the same value, these options set a gas price of 1ubloc.
	...
	amountDeducted := balanceBefore - balanceAfter - ublocToSend
